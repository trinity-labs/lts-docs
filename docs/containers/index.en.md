...
This page inventories every `docker-compose.yml` and `Dockerfile` found in the Docker source trees currently mounted here.

The scanned roots are `TRINITY-DOCKER/var/docker` and `TRINITY-CLOUD/mnt/ENTREPRISE/DOCKER`.

This public version keeps the same manifests, but redacts inline secrets, passwords, and secret file paths.

Inventory: 32 paths found, 30 distinct contents.
...
Note: Identical content for the paths below.
