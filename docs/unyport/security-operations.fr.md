...
`UnyPort` combine une surface web durcie avec un jeu volontairement restreint d'actions operationnelles en ecriture. L'objectif est de laisser les opérateurs inspecter, vérifier et ouvrir certains outils sans transformer le portail en porte d'administration incontrolee.
...
Le modele reste compact, mais il est cohérent et visible dans les chemins reellement utilises par le portail.
...
Elle sert de resume opérateur, pas de remplacement a un audit complet.
...
Ce n'est pas encore le lieu d'un cycle de vie complet de VM ni d'une orchestration de cluster. Le README public presente explicitement le produit actuel comme une `V1`, orientée monitoring d'abord, avec des workflows Xen plus larges attendus plus tard.

## Logs et limites de support
Le portail peut exposer certains fichiers de log, l'état des services et des signaux de sécurité hote. Lorsque le besoin depasse ce perimetre :
